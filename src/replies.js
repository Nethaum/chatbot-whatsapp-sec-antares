...
const timeLabelPattern = '(?:hor[aá]rios?(?:\\s+de\\s+in[ií]cio)?|in[ií]cio)\\s*(?:[:\\-]|\\bas\\b|\\b[aà]s\\b)?\\s*';
const reservationTimeWithLabelGlobalRegex = new RegExp(`\\b(?:${timeLabelPattern})?${reservationTimePattern}\\b`, 'gi');
...
async function handleActiveReservationState(input, text, state, club, chatId, context = {}) {
...
function handleReservationDetailsInput(input, state, club, chatId, context = {}) {
  const { choice } = state;
  const text = normalizeText(input);
...
function extractDisplayPhoneDigits(value) {
...
function formatPhoneForDisplay(value) {
  ...
  if (nationalDigits.length === 10 || nationalDigits.length === 11) {
    ...
    return `${digits.startsWith('55') ? '+55 ' : ''}${ddd} ${formatSubscriberNumber(subscriber)}`;
  }
...
function extractReservationName(value) {
  ...
  const withoutDatesAndTimes = stripDateMentions(relevantText).replace(reservationTimeWithLabelGlobalRegex, ' ');
